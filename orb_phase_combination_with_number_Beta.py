...
def initialize_kalman_filter():
    ...
    # increase initial P (alphaV = 0.1)
    kf.Q = np.eye(4, dtype=float) * 0.15

    # Measurement Noise Covariance (R)
    
    # decrease R from 5 to 3
    kf.R = np.eye(2, dtype=float) * 3.0
...
def adaptive_kalman_update(kf, z):
    ...
    # add baseline noise
    baseline= np.eye(2,dtype=float)*0.05
    kf.R = kf.alpha_R * kf.R + (1 - kf.alpha_R) * innovation_cov+baseline
...
def estimate_shift_combined(img1, img2):
    ...
    # dynamic weight for phase correlation 
    weight_pc = min(num_matches, 10)
